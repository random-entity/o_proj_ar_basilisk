...
const QFmt q_fmt{[] {
  ...
  fmt.fault = kInt8;
  fmt.extra[0] = {.register_number = kControlVelocityError,
                  .resolution = kFloat};
  fmt.extra[1] = {.register_number = kEncoder1Velocity, .resolution = kFloat};
  fmt.extra[2] = {.register_number = kEncoderValidity, .resolution = kInt8};
  return fmt;
}()};
